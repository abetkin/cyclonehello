...
class ColorFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if record.exc_info:
            record.exc_text = self.color(record.levelno,
                                         '>> %s' % self.formatException(record.exc_info).replace('\n', '\n>> '))
...
class MonastHTTP(resource.Resource):
    ...
    def _onRequestFailure(self, reason, request):
        ...
        log.error("HTTP Request from %s:%s (%s) to %s failed: %s", request.client.host, request.client.port,
                  session.uid, request.uri, reason.getErrorMessage())
    ...
    def render_GET(self, request):
        session = request.getSession()
        session.touch()
        log.debug("HTTP Request from %s:%s (%s) to %s", request.client.host, request.client.port, session.uid,
                  request.uri)
    ...
    def doAuthentication(self, request):
        ...
        if success:
            log.log(logging.NOTICE,
                    "User \"%s\" Successful Authenticated with Session \"%s\"" % (username, session.uid))
    ...
    def getStatus(self, request):
        ...
        for tech, peerlist in server.status.peers.items():
            ...
            tmp[servername]['peers'][tech].sort(
                lambda x, y: cmp(x.get(self.monast.sortPeersBy), y.get(self.monast.sortPeersBy)))
        ## Channels
...
class MonastAMIProtocol(manager.AMIProtocol):
    ...
    def redirect(self, channel, context, exten, priority, extraChannel=None, extraContext=None, extraExten=None,
                 extraPriority=None):
...
class Monast(object):
    ...
    def __connected__(self, ami, servername):
        ...
        server.pushTask(server.ami.command, 'core show version') \
            .addCallbacks(_onCoreShowVersion, self._onAmiCommandFailure,
                          errbackArgs=(servername, "Error Requesting Asterisk Version"))
    ...
    def _createPeer(self, servername, **kw):
        ...
        if not server.status.peers.has_key(channeltype) and kw.get('forced', False):
            log.warning("Server %s :: Adding a not implemented ChannelType %s (forced in config file)", servername,
                        channeltype)
            ...
            if channeltype == 'DAHDI':
                ...
                if peer.callerid == "--":
                    if peer.peername.isdigit():
                        peer.callerid = [peer.channel, "%s %02d" % (peer.signalling, int(peer.peername))][
                            peer.callerid == '--']
                    elif peer.signalling:
                        peer.callerid = [peer.channel, "%s %s" % (peer.signalling, peer.peername)][
                            peer.callerid == '--']
    ...
    def _updatePeer(self, servername, **kw):
        ...
        try:
                ...
                for k, v in kw.items():
                    ...
                    if k not in ('_log', '_action'):
                        if peer.__dict__.has_key(k):
                            ...
                            log.warning("Server %s :: User/Peer %s/%s does not have attribute %s", servername,
                                        channeltype, peername, k)
            ...
            log.exception("Server %s :: Unhandled exception updating User/Peer: %s/%s", servername, channeltype,
                          peername)
                ...
                for k, v in kw.items():
                    if k not in ('_log', '_action'):
                        if chan.__dict__.has_key(k):
                            ...
                            log.warning("Server %s :: Channel %s (%s) does not have attribute %s", servername, uniqueid,
                                        chan.channel, k)
                self.http._addUpdate(servername=servername, subaction='Update', **chan.__dict__.copy())
                if kw.get("_action") == "updateDahdiCallsCounter":
                    self._updatePeer(servername, channeltype="DAHDI", peername=kw.get("dahdichannel"),
    ...
    def _createBridge(self, servername, **kw):
        ...
        if not server.status.bridges.has_key(bridgekey):
            if not server.status.channels.has_key(uniqueid):
                log.warning("Server %s :: Could not create bridge %s (%s) with %s (%s). Source Channel not found.",
                            servername, uniqueid, channel, bridgeduniqueid, bridgedchannel)
                return False
            if not server.status.channels.has_key(bridgeduniqueid):
                log.warning("Server %s :: Could not create bridge %s (%s) with %s (%s). Bridged Channel not found.",
                            servername, uniqueid, channel, bridgeduniqueid, bridgedchannel)
                return False
            ...
            log.debug("Server %s :: Bridge create: %s (%s) with %s (%s) %s", servername, uniqueid, channel,
                      bridgeduniqueid, bridgedchannel, _log)
            ...
            log.warning("Server %s :: Bridge already exists: %s (%s) with %s (%s)", servername, uniqueid, channel,
                        bridgeduniqueid, bridgedchannel)
        return False
    ...
    def _updateBridge(self, servername, **kw):
        ...
        try:
            ...
            if bridge:
                log.debug("Server %s :: Bridge update: %s (%s) with %s (%s) %s", servername, bridge.uniqueid,
                          bridge.channel, bridge.bridgeduniqueid, bridge.bridgedchannel, _log)
                for k, v in kw.items():
                    if k not in ('_log', '_bridge'):
                        if bridge.__dict__.has_key(k):
                            bridge.__dict__[k] = v
                        else:
                            log.warning("Server %s :: Bridge %s (%s) with %s (%s) does not have attribute %s",
                                        servername, uniqueid, bridge.channel, bridgeduniqueid, bridge.bridgedchannel, k)
                bridge.seconds = int(time.time() - bridge.linktime)
                self.http._addUpdate(servername=servername, subaction='Update', **bridge.__dict__.copy())
                if logging.DUMPOBJECTS:
                    log.debug("Object Dump:%s", bridge)
            else:
                log.warning("Server %s :: Bridge not found: %s (%s) with %s (%s)", servername, uniqueid, channel,
                            bridgeduniqueid, bridgedchannel)
        except:
            log.exception("Server %s :: Unhandled exception updating bridge: %s (%s) with %s (%s)", servername,
                          uniqueid, channel, bridgeduniqueid, bridgedchannel)
    ...
    def _removeBridge(self, servername, **kw):
        ...
        try:
            ...
            if bridge:
                log.debug("Server %s :: Bridge remove: %s (%s) with %s (%s) %s", servername, uniqueid, bridge.channel,
                          bridge.bridgeduniqueid, bridge.bridgedchannel, _log)
                if kw.get('_isLostBridge'):
                    log.warning("Server %s :: Removing lost bridge: %s (%s) with %s (%s)", servername, uniqueid,
                                bridge.channel, bridge.bridgeduniqueid, bridge.bridgedchannel)
                del server.status.bridges[bridgekey]
                self.http._addUpdate(servername=servername, action='RemoveBridge', uniqueid=uniqueid,
                                     bridgeduniqueid=bridgeduniqueid)
                if logging.DUMPOBJECTS:
                    log.debug("Object Dump:%s", bridge)
            else:
                log.warning("Server %s :: Bridge does not exists: %s (%s) with %s (%s)", servername, uniqueid, channel,
                            bridgeduniqueid, bridgedchannel)
        except:
            log.exception("Server %s :: Unhandled exception removing bridge: %s (%s) with %s (%s)", servername,
                          uniqueid, channel, bridgeduniqueid, bridgedchannel)
    ...
    def _createParkedCall(self, servername, **kw):
        ...
        if not parked:
            ...
            if not self.isParkedCallStatus:
                log.warning("Server %s :: ParkedCall already exists: %s at %s", servername, parked.channel,
                            parked.exten)
        ...
        try:
            ...
            if queue:
                ...
                if event in ("QueueMember", "QueueMemberAdded", "QueueMemberStatus", "QueueMemberPaused"):
                    ...
                    if not member:
                        log.debug("Server %s :: Queue update, member added: %s -> %s %s", servername, queuename,
                                  location, _log)
                        ...
                        log.debug("Server %s :: Queue update, member updated: %s -> %s %s", servername, queuename,
                                  location, _log)
                        member.name = membername
                        member.queue = kw.get('queue')
                        member.callstaken = kw.get('callstaken', 0)
                        ...
                        member.membership = kw.get('membership')
                        member.paused = kw.get('paused')
                        member.pausedat = [member.pausedat, time.time()][
                            event == "QueueMemberPaused" and member.paused == '1']
                        member.pausedur = int(time.time() - member.pausedat)
                        member.penalty = kw.get('penalty')
                        member.status = kw.get('status')
                        member.statustext = AST_DEVICE_STATES.get(member.status, 'Unknown')
                ...
                if event == "QueueMemberRemoved":
                    ...
                    member = server.status.queueMembers.get(memberid)
                    if member:
                        log.debug("Server %s :: Queue update, member removed: %s -> %s %s", servername, queuename,
                                  location, _log)
                        del server.status.queueMembers[memberid]
                        self.http._addUpdate(servername=servername, action='RemoveQueueMember',
                                             location=member.location, queue=member.queue)
                        if logging.DUMPOBJECTS:
                            log.debug("Object Dump:%s", member)
                    else:
                        log.warning("Server %s :: Queue Member does not exists: %s -> %s", servername, queuename,
                                    location)
                    return
                ...
                if event in ("QueueEntry", "Join"):
                    ...
                    if not client:
                        log.debug("Server %s :: Queue update, client added: %s -> %s %s", servername, queuename,
                                  uniqueid, _log)
                        ...
                        log.debug("Server %s :: Queue update, client updates: %s -> %s %s", servername, queuename,
                                  uniqueid, _log)
                        client.channel = kw.get('channel')
                        client.queue = kw.get('queue')
                        client.calleridname = kw.get('calleridname')
                ...
                if event == "QueueCallerAbandon":
                    ...
                    client = server.status.queueClients.get(clientid)
                    if client:
                        log.debug("Server %s :: Queue update, client marked as abandonned: %s -> %s %s", servername,
                                  queuename, uniqueid, _log)
                        client.abandonned = True
                        queue.abandoned += 1
                        self.http._addUpdate(servername=servername, subaction='Update', **queue.__dict__.copy())
                    else:
                        log.warning("Server %s :: Queue Client does not exists: %s -> %s", servername, queuename,
                                    uniqueid)
                    return
                ...
                if event == "Leave":
                    ...
                    if client:
                        ...
                        log.debug("Server %s :: Queue update, client removed: %s -> %s %s", servername, queuename,
                                  uniqueid, _log)
                        del server.status.queueClients[clientid]
                        self.http._addUpdate(servername=servername, action='RemoveQueueClient',
                                             uniqueid=client.uniqueid, queue=client.queue)
                        if logging.DUMPOBJECTS:
                            log.debug("Object Dump:%s", client)
                    else:
                        log.warning("Server %s :: Queue Client does not exists: %s -> %s", servername, queuename,
                ...
                if (self.displayQueuesDefault and not server.displayQueues.has_key(queuename)) or (
                    not self.displayQueuesDefault and server.displayQueues.has_key(queuename)):
    ...
    def __parseMonastConfig(self):
        ...
        try:
            ...
            if not self.sortPeersBy in ('channel', 'callerid'):
                log.error(
                    "Invalid value for 'sortby' in section 'peers' of config file. valid options: channel, callerid")
        ...
        for user, display in config.items('peers'):
            ...
            if tech in server.status.peers.keys():
                if (self.displayUsersDefault and display == 'hide') or (
                    not self.displayUsersDefault and display == 'show'):
        ...
        for queue, display in config.items('queues'):
            ...
            if (self.displayQueuesDefault and display == 'hide') or (
                not self.displayQueuesDefault and display == 'show'):
    ...
    def _onAmiCommandFailure(self, reason, servername, message=None):
        ...
        if type(reason.value) == AMICommandFailure and type(reason.value.args[0]) == type(dict()) and reason.value.args[
            0].has_key('message'):
    ...
    def _requestAsteriskConfig(self, servername):
            ...
            def onIax2ShowPeers(result):
                if len(result) > 2:
                    for line in result[1:][:-1]:
                        ...
                        self.handlerEventPeerEntry(server.ami,
                                                   {'channeltype': 'IAX2', 'objectname': peername, 'status': 'Unknown'})

            log.debug("Server %s :: Requesting IAX Peers (via iax2 show peers)..." % servername)
            server.pushTask(server.ami.command, 'iax2 show peers') \
                .addCallbacks(onIax2ShowPeers, self._onAmiCommandFailure,
                              errbackArgs=(servername, "Error Requesting IAX Peers (via iax2 show peers)"))
        else:
        ...
        server.pushTask(server.ami.collectDeferred, {'action': 'dahdishowchannels'}, 'DAHDIShowChannelsComplete') \
            .addCallbacks(onDahdiShowChannels, onDahdiShowChannelsFailure,
                          errbackArgs=(servername, "Error Requesting DAHDI Channels"))
        ...
        server.pushTask(server.ami.command, 'khomp channels show') \
            .addCallbacks(onKhompChannelsShow, self._onAmiCommandFailure,
                          errbackArgs=(servername, "Error Requesting Khomp Channels"))
        ...
        def onGetMeetmeConfig(result):
            ...
            for k, v in result.items():
                if v.startswith("conf="):
                    ...
                    if (self.displayMeetmesDefault and not server.displayMeetmes.has_key(meetmeroom)) or (
                        not self.displayMeetmesDefault and server.displayMeetmes.has_key(meetmeroom)):
        ...
        server.pushTask(server.ami.getConfig, 'meetme.conf') \
            .addCallbacks(onGetMeetmeConfig, self._onAmiCommandFailure,
                          errbackArgs=(servername, "Error Requesting meetme.conf"))
        ...
        server.pushTask(server.ami.collectDeferred, {'Action': 'QueueStatus'}, 'QueueStatusComplete') \
            .addCallbacks(onQueueStatus, self._onAmiCommandFailure,
                          errbackArgs=(servername, "Error Requesting Queue Status"))
        ...
        def onCustomPeerStatus(events):
            ...
            for line in events:
                if line.lower().startswith("/monast/peerstatus/"):
                    ...
                    self._updatePeer(servername, channeltype=tech, peername=peer, customStatus=value,
                                     _log="Custom Peer Status on ASTDB")

        log.debug("Server %s :: Requesting Custom Peer Status from ASTDB..." % servername)
        server.pushTask(server.ami.command, "database show") \
            .addCallbacks(onCustomPeerStatus, self._onAmiCommandFailure,
                          errbackArgs=(servername, "Error Requesting Custom Peer Status from ASTDB"))
    ...
    def taskCheckStatus(self, servername):
        ...
        def onStatusComplete(events):
            ...
            for uniqueid, channel in lostChannels:
                self._removeChannel(servername, uniqueid=uniqueid, channel=channel, _isLostChannel=True,
                                    _log="-- Lost Channel")
            ...
            lostBridges = [
                ...
                if
                not server.status.channels.has_key(b.uniqueid) or not server.status.channels.has_key(b.bridgeduniqueid)
            ]
            for uniqueid, bridgeduniqueid in lostBridges:
                self._removeBridge(servername, uniqueid=uniqueid, bridgeduniqueid=bridgeduniqueid, _isLostBridge=True,
                                   _log="-- Lost Bridge")
            ...
            for channeltype, peers in server.status.peers.items():
                for peername, peer in peers.items():
                    ...
                    if peer.calls != calls:
                        log.warning(
                            "Server %s :: Updating %s/%s calls counter from %d to %d, we lost some AMI events...",
                            servername, channeltype, peername, peer.calls, calls)
                        self._updatePeer(servername, channeltype=channeltype, peername=peername, calls=calls,
                                         _log="-- Update calls counter (by status request)")

            log.debug("Server %s :: End of channels status..." % servername)

        server.pushTask(server.ami.status) \
            .addCallbacks(onStatusComplete, self._onAmiCommandFailure,
                          errbackArgs=(servername, "Error Requesting Channels Status"))
        ...
        for queuename in server.status.queues.keys():
            server.pushTask(server.ami.collectDeferred, {'Action': 'QueueStatus', 'Queue': queuename},
                            'QueueStatusComplete') \
                .addCallbacks(onQueueStatusComplete, self._onAmiCommandFailure,
                              errbackArgs=(servername, "Error Requesting Queues Status"))
            ...
            if handler:
                if self.authRequired:
                    if role in self.authUsers[session.username].servers.get(servername):
                        ...
                        self.http._addUpdate(servername=servername, sessid=session.uid, action="RequestError",
        ...
        for idx, originate in enumerate(originates):
            ...
            server.pushTask(server.ami.originate, *originate) \
                .addErrback(self._onAmiCommandFailure, servername,
                            "Error Executting Client Action Originate: %s" % (logs[idx]))
    ...
    def clientAction_Transfer(self, session, action):
        ...
        log.info(
            "Server %s :: Executting Client Action Transfer: %s -> %s@%s..." % (servername, channel, exten, context))
        server.pushTask(server.ami.redirect, channel, context, exten, priority, extraChannel, extraContext, extraExten,
                        extraPriority) \
            .addErrback(self._onAmiCommandFailure, servername,
                        "Error Executting Client Action Transfer: %s -> %s@%s" % (channel, exten, context))
    ...
    def clientAction_Park(self, session, action):
        ...
        server.pushTask(server.ami.park, channel, announce, "") \
            .addErrback(self._onAmiCommandFailure, servername,
                        "Error Executting Client Action Transfer: %s from %s" % (channel, announce))
    ...
    def clientAction_RequestInfo(self, session, action):
        ...
        def _onResponse(response):
            self.http._addUpdate(servername=servername, sessid=session.uid, action="RequestInfoResponse",
                                 response=response)
    ...
    def clientAction_MonitorStart(self, session, action):
        ...
        server.pushTask(server.ami.monitor, channel, "", "", 1) \
            .addErrback(self._onAmiCommandFailure, servername,
                        "Error Executting Monitor Start on Channel: %s" % channel)
    ...
    def clientAction_QueueMemberPause(self, session, action):
        ...
        log.info(
            "Server %s :: Executting Client Action Queue Member Pause: %s -> %s..." % (servername, queue, location))
        server = self.servers.get(servername)
        server.pushTask(server.ami.queuePause, queue, location, True) \
            .addErrback(self._onAmiCommandFailure, servername,
                        "Error Executting Queue Member Pause: %s -> %s" % (queue, location))
    ...
    def clientAction_QueueMemberUnpause(self, session, action):
        ...
        log.info(
            "Server %s :: Executting Client Action Queue Member Unpause: %s -> %s..." % (servername, queue, location))
        server = self.servers.get(servername)
        server.pushTask(server.ami.queuePause, queue, location, False) \
            .addErrback(self._onAmiCommandFailure, servername,
                        "Error Executting Queue Member Unpause: %s -> %s" % (queue, location))
    ...
    def clientAction_QueueMemberAdd(self, session, action):
        ...
        server.pushTask(server.ami.queueAdd, queue, location, penalty, False, membername, stateIface) \
            .addErrback(self._onAmiCommandFailure, servername,
                        "Error Executting Queue Member Add: %s -> %s" % (queue, location))
    ...
    def clientAction_QueueMemberRemove(self, session, action):
        ...
        log.info(
            "Server %s :: Executting Client Action Queue Member Remove: %s -> %s..." % (servername, queue, location))
        server = self.servers.get(servername)
        server.pushTask(server.ami.queueRemove, queue, location) \
            .addErrback(self._onAmiCommandFailure, servername,
                        "Error Executting Queue Member Remove: %s -> %s" % (queue, location))
    ...
    def clientAction_SpyChannel(self, session, action):
        ...
        server.pushTask(server.ami.originate, channel, context, exten, priority, timeout, callerid, account,
                        application, data, variable, async) \
            .addErrback(self._onAmiCommandFailure, servername,
                        "Error Executting Client Spy Channel: %s -> %s" % (spyer, spyee))
        ...
        if UserEvent == "MonastEvent":
            ...
            if MonastEvent == "PeerStatus":
				...
				* To Clear Custom Peer Status:
                ...
                self._updatePeer(ami.servername, channeltype=tech, peername=peer, customStatus=status,
                                 _log="change status by UserEvent")
    ...
    def handlerEventAlarm(self, ami, event):
        ...
        self._updatePeer(ami.servername, channeltype=tech, peername=chan, alarm=alarm, status=alarm,
                         _log="Alarm Detected (%s)" % alarm)
    ...
    def handlerEventAlarmClear(self, ami, event):
        ...
        self._updatePeer(ami.servername, channeltype=tech, peername=chan, alarm='No Alarm', status='No Alarm',
                         _log="Alarm Cleared")
    ...
    def handlerEventRename(self, ami, event):
        ...
        self._updateChannel(ami.servername, uniqueid=uniqueid, channel=newname,
                            _log="Channel %s renamed to %s" % (channel, newname))
        bridgekey = self._locateBridge(ami.servername, uniqueid=uniqueid)
        if bridgekey:
            if uniqueid == bridgekey[0]:
                self._updateBridge(ami.servername, uniqueid=bridgekey[0], bridgeduniqueid=bridgekey[1], channel=newname,
                                   _log="Channel %s renamed to %s" % (channel, newname))
            else:
                self._updateBridge(ami.servername, uniqueid=bridgekey[0], bridgeduniqueid=bridgekey[1],
                                   bridgedchannel=newname, _log="Channel %s renamed to %s" % (channel, newname))
    ...
    def handlerEventMasquerade(self, ami, event):
        ...
        if not cloneUniqueid:
            log.warn(
                "Server %s :: Detected BUG on Asterisk. Masquerade Event does not have cloneuniqueid and originaluniqueid properties. " % ami.servername \
    ...
    def handlerEventHangup(self, ami, event):
        ...
        if queueCall:
            log.debug("Server %s :: Queue update, call hangup: %s -> %s", ami.servername, queueCall.client.get('queue'),
                      uniqueid)
            del server.status.queueCalls[uniqueid]
            if queueCall.member:
                self.http._addUpdate(servername=ami.servername, action="RemoveQueueCall", uniqueid=uniqueid,
                                     queue=queueCall.client.get('queue'), location=queueCall.member.get('location'))
        ...
        for qname, clientuniqueid in server.status.queueClients.items():
            if clientuniqueid == uniqueid:
                self._updateQueue(ami.servername, queue=qname, event="Leave", uniqueid=uniqueid,
                                  _log="By Channel Hangup")
    ...
    def handlerEventDial(self, ami, event):
        ...
        if subevent.lower() == 'begin':
            ...
            if bridgekey:
                self._removeBridge(ami.servername, uniqueid=bridgekey[0], bridgeduniqueid=bridgekey[1],
                                   _log="-- Dial End")
            ...
            if queueCall:
                ...
                if queueCall.member:
                    log.debug("Server %s :: Queue update, client -> member call unlink: %s -> %s -> %s", ami.servername,
                              queueCall.client.get('queue'), uniqueid, queueCall.member.get('location'))
                    self.http._addUpdate(servername=ami.servername, action="RemoveQueueCall", uniqueid=uniqueid,
                                         queue=queueCall.client.get('queue'), location=queueCall.member.get('location'))
                    if logging.DUMPOBJECTS:
                        log.debug("Object Dump:%s", queueCall)
    ...
    def handlerEventLink(self, ami, event):
        ...
        if queueCall:
            ...
            for location in [location, "%s/n" % location]:
                ...
                if member:
                    log.debug("Server %s :: Queue update, client -> member call link: %s -> %s -> %s", ami.servername,
                              queuename, uniqueid, location)
    ...
    def handlerEventMonitorStart(self, ami, event):
        ...
        self._updateChannel(ami.servername, uniqueid=event.get('uniqueid'), channel=event.get('channel'), monitor=True,
    ...
    def handlerEventMonitorStop(self, ami, event):
        ...
        self._updateChannel(ami.servername, uniqueid=event.get('uniqueid'), channel=event.get('channel'), monitor=False,
